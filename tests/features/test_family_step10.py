"""Step 10 tests: correlation + entropy families parity."""
...
from src.features.config import WINDOWS_CORR, WINDOWS_PENTROPY
...
def bars_with_base() -> pd.DataFrame:
    rng = np.random.default_rng(2024_05_13)
    n = 30_000
...
def _assert_parity(legacy_pd, engine_pl, feature_cols, warmup):
    ...
    for col in feature_cols:
        ...
        assert np.array_equal(legacy_nan, engine_nan), f"{col}: NaN positions differ"
        valid = ~legacy_nan
        if valid.any():
...
def test_correlation_family_parity(bars_with_base):
    legacy = utils.compute_correlations(bars_with_base, WINDOWS_CORR)
    legacy_cols = [c for c in legacy.columns if c not in bars_with_base.columns]
    assert len(legacy_cols) == 4 * len(WINDOWS_CORR)

    bars_pl = _to_polars(bars_with_base)
    engine = FeatureEngine(tiers=(1,), families=("correlation",))
    result = engine.transform(bars_pl)
    _assert_parity(legacy, result.data, legacy_cols, result.warmup_trimmed)


def test_entropy_family_parity(bars_with_base):
    legacy = utils.compute_permutation_entropy(bars_with_base, WINDOWS_PENTROPY)
    legacy_cols = [c for c in legacy.columns if c not in bars_with_base.columns]
    assert len(legacy_cols) == len(WINDOWS_PENTROPY)

    bars_pl = _to_polars(bars_with_base)
    engine = FeatureEngine(tiers=(1,), families=("entropy",))
    result = engine.transform(bars_pl)
    _assert_parity(legacy, result.data, legacy_cols, result.warmup_trimmed)
